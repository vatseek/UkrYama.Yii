<?php
    $this->pageTitle=Yii::app()->name . ' :: Благодарности';
    $this->layout='//layouts/header_blank';
?>

 <h2>Все те, кто так или иначе помог УкрЯме</h2>
 	
  <p>Разработка:
    <br />
   		</p>

  <ul class="default"> 			
    <li><a href="http://navalny.ru/" alt="Финальная битва между добром и нейтралитетом" target="_blank" >Алексей Навальный</a> &mdash; идея, создание <a href="http://rosyama.ru/" alt="Яму мне запили!" target="_blank" >РосЯмы</a>;</li>
   			
    <li>Команда веб-студии &laquo;<a href="http://greensight.ru/" alt="Главная страница сайта студии «Гринсайт&raquo;" target="_blank" >Greensight</a>» <i>(отдельное спасибо Дмитрию Никифорову)</i>;</li>
   			
    <li>Сергей Рыжиков — генеральный директор «<a href="http://www.1c-bitrix.ru/" alt="Система управления интернет-проектами" target="_blank" >1С-Битрикс</a>»;</li>
   			
    <li>Хостинг-компания — «<a href="http://ukraine.com.ua/" alt="Профессиональный хостинг" target="_blank" >Украина</a>», <noindex>«<a href="http://ihc.com.ua/" target="_blank" >Интернет-хостинг центр</a>»</noindex>;</li>
   			
    <li><a href="http://www.free-lance.ru/users/anmari" alt="Ан Мари" target="_blank" >Марианна</a> нарисовала отличный логотип;</li>
   			
    <li>Веб-студия <a href="http://stfalcon.com/" alt="Разработка веб-проектов" target="_blank" >stfalcon.com</a> — команда профессионалов, которая любит свою работу и нацелена на качественный результат;</li>
   			
    <li>Спасибо <a href="http://habrahabr.ru/users/vessi/" alt="Системный администратор" target="_blank" >Михаилу Бортнику</a> за приложение «УкрЯма» для ОС Android;</li>
   			
    <li>Некто Александр помогает в <a href="http://www.our-dns.info/" title="Удаленное администрирование серверов" alt="Удаленное администрирование серверов" target="_blank" >администрировании серверов</a>.</li>
   		</ul>
 	
  <p></p>
 	
  <br />
 	
  <p>Финансово (<a href="http://ukryama.info/blog/reports/" target="_blank" >отчеты о расходах</a>):</p>
 		
  <table width="60%" border="0" cellspacing="2" cellpadding="4"> 	
    <tbody>
      <tr> 		<td style="border-image: initial; "><strong>Имя</strong></td> 		<td style="border-image: initial; "><strong>Сумма&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong></td> 		<td style="border-image: initial; "><strong>Примечание&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong></td> 		<td style="border-image: initial; "><strong>Дата</strong></td> 	</tr>
     	
      <tr> 		<td style="border-image: initial; ">Петр Нек</td> 		<td style="border-image: initial; ">100 грн</td> 		<td style="border-image: initial; ">donate</td> 		<td style="border-image: initial; ">26.12.2011</td> 	</tr>
     	
      <tr> 		<td style="border-image: initial; ">Сергей Думинский</td> 		<td style="border-image: initial; ">&nbsp;&nbsp;20 грн</td> 		<td style="border-image: initial; ">donate</td> 		<td style="border-image: initial; ">12.12.2011</td> 	</tr>
     	
      <tr> 		<td style="border-image: initial; ">Андрейчин Игорь и Марина Ясинецкая</td> 		<td style="border-image: initial; ">995 руб</td> 		<td style="border-image: initial; ">На развитие УкрЯмы</td> 		<td style="border-image: initial; ">18.02.2012</td> 	</tr>
     		
      <tr> 		<td style="border-image: initial; "><noindex><a href="http://www.facebook.com/profile.php?id=100000644136805" target="_blank" rel="nofollow" >Павел Сиделев</a></noindex></td> 		<td style="border-image: initial; ">&nbsp;&nbsp;50 грн</td> 		<td style="border-image: initial; ">На развитие УкрЯмы</td> 		<td style="border-image: initial; ">18.02.2012</td> 	</tr>
     	</tbody>
  </table>
 	
  <br />
 	
  <p>Мы будем рады любой помощи в реализации проекта: <a href="mailto:info@ukryama.com" title="Написать письмо" >info@ukryama.com</a></p>
 	
  <br />

  <br />
 	
  <p><a href="http://navalny.ru/" target="_blank" ><img src="/images/pages/thanks/navalny_logo.png" style="padding:0px 10px 0px 0px;"  /></a><a href="http://greensight.ru/" target="_blank" ><img src="/images/pages/thanks/greensight_logo.png" style="padding:0px 10px;"  /></a><a href="http://www.1c-bitrix.ru/" target="_blank" ><img src="/images/pages/thanks/1c-bitrix-logo.png" style="padding:0px 10px;"  /></a><a href="http://ukraine.com.ua/" target="_blank" ><img src="/images/pages/thanks/ukraine_logo.png" style="padding:0px 10px;"  /></a><a href="http://stfalcon.com/" target="_blank" ><img src="/images/pages/thanks/stfalcon_logo.png" style="padding:0px 10px;"  /></a></p>

  <br />
 	
  <p><a href="https://homemoney.ua" target="_blank" ><img src="/images/pages/thanks/money.png" style="padding:0px 10px 0px 0px;"  /></a></p>